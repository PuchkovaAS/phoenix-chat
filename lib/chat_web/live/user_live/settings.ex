defmodule ChatWeb.UserLive.Settings do
  ...
  def render(assigns) do
    ...
    <!-- ✅ Отображение ошибок -->
        <%= for {msg, _} <- @nickname_form[:nickname].errors do %>
          <p class="text-error text-sm mt-1">{msg}</p>
        <% end %>

        <.button variant="primary" phx-disable-with="Saving...">Save Nickname</.button>
      </.form>
  ...
  def mount(_params, _session, socket) do
    ...
    # ✅ Создаём changeset для nickname
    nickname_changeset = Accounts.change_user_profile(user, %{})
  ...
  # Существующие обработчики для email...
  @impl true
  ...
  # Существующие обработчики для password...
